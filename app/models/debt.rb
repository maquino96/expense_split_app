class Debt < ApplicationRecord
  ...
  def self.something_about_working_out_duplicates_or_whatever     # needs to be a class method ? i think so
    ...
    Debt.all.each do |this_debt|
        ...
        if amount > this_debt.amount
            Date.create(event: Event.find_by(name: "The Universal Event"), creditor: this_debt.creditor, debtor: this_debt.debtor, amount: amount) #event must be nil because this new debt was created from multiple events, right ?
            this_debt.destroy
        end
    end
    ...
    Debt.all.each do |this_debt|
        ...
        Debt.all.each do |that_debt|
            if this_debt.debtor == that_debt.creditor && this_debt.creditor == that_debt.debtor
                ...
                if amount > 0
                    Debt.create(event: Event.find_by(name: "The Universal Event"), creditor: this_debt.creditor, debtor: this_debt.debtor, amount: amount)
                elsif amount < 0
                    Debt.create(event: Event.find_by(name: "The Universal Event"), creditor: that_debt.creditor, debtor: that_debt.debtor, amount: -amount)
